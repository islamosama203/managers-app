...
struct BookingCountView: View {
    ...
    var body: some View {
        ZStack {
            ...
            .onAppear {
            viewModel.getAllCategoriesLoanCount()
            selectToday()
        }
    ...
    var content: some View {
        VStack {
            NavigationBarView(title: "Booking Count", container: viewModel.container)
            ...
            if viewModel.selectedReport.isEmpty {
                ...
                ScrollView(showsIndicators: false) {
                    ForEach(viewModel.selectedReport, id: \.self) { report in
                        CategoryCountCellView(category: report)
                            .onTapGesture {
                            viewModel.container.appState[\.currentRoute] = .merchantCount(categoryId: report.categoryCode)
                        }
                    }.padding(.bottom)
                }
            }
...
#if DEBUG
    struct LoansCountView_Previews: PreviewProvider {
        static var previews: some View {
            BookingCountView(viewModel: BookingCountView.ViewModel(container: .preview))
        }
    }
#endif
